...
class App extends Component {
  ...
  checkGuess = (name, cb) => {
    ...
    if (newState.pickedChars.includes(name)) {
      newState.alertMessage = `Incorrect, you already picked "${name.toUpperCase()}"!`
  ...
  render() {
    return (
      <div>
        <NavBar style={{ background: "#2866C5", marginBottom: "5px" }} />

        <GridMDC container direction="column" style={{ margin: "0 auto", maxWidth: 945 }}>

          <GridMDC item lg={12}>
            <PaperMDC>
              {this.state.alertMessage === "You guessed correctly!" ? (
